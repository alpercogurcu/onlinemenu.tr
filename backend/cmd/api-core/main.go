...
func main() {
	...
	if err := app.Start(startCtx); err != nil {
		fmt.Fprintf(os.Stderr, "api-core: start: %v\n", err)
		os.Exit(1)
	}
	...
	if err := app.Stop(stopCtx); err != nil {
		fmt.Fprintf(os.Stderr, "api-core: stop: %v\n", err)
		os.Exit(1)
	}
}
...
// auth.RequirePermission (identity/tenant/party/hr-core route registration)
// answers 403 for every caller — the module authz work is otherwise inert here.
...
func newContextTokenSigner() (*auth.ContextTokenSigner, error) {
	...
	if secret == "" {
		return nil, errors.New("api-core: CTX_TOKEN_SECRET env var is required")
	}
...
func newTokenVerifier() (auth.TokenVerifier, error) {
	...
	if issuerURL == "" || audience == "" {
		return nil, fmt.Errorf("api-core: KEYCLOAK_ISSUER_URL and KEYCLOAK_AUDIENCE are required when APP_ENV=%q", env)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("api-core: build keycloak verifier: %w", err)
	}
	return verifier, nil
}
