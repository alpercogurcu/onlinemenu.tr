...
// authorization for inventory's branch-scoped write actions (ADR-DATA-006,
// docs/lessons-from-b2b.md item 6). Same-tenant isolation across branches was
// the gap this closes: RLS (layer 1) only isolates by tenant_id, and OPA
// (layer 2) only decides whether the action is allowed at all plus a coarse
// scope — it never sees the specific branch_id of the resource being acted
// on. That check belongs here.
//
...
// branch-forbidden caller receives 403, never a 409 that would otherwise
// leak the resource's current status to someone who has no business acting
// on it.
