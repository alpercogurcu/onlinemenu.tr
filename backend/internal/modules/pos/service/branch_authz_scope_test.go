...
// managerRoleID / cashierRoleID mirror the well-known system role ids in
// configs/opa/bundles/authz.rego (inventory/service/branch_authz_scope_test.go
// uses the same manager id; cashier is pos's counter-facing branch-scoped role).
var (
...
func TestRequireBranch_ManagerScopeExemptsEvenWithMismatchedPrincipalBranch(t *testing.T) {
	...
	var gotErr error
	mw := auth.RequirePermission(engine, "pos.check.close")
...
func TestRequireBranch_BranchRoleMustMatchResourceBranch(t *testing.T) {
	...
	mw := auth.RequirePermission(engine, "pos.check.close")
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotErr = service.RequireBranchForTest(r.Context(), cashier, otherBranch)
	})
