...
1. **Kısmi onay + kısmi sevk kombinasyonu** — `approved_qty < requested_qty` ve çok-shipment birlikte: `closed`'ın tam mı yoksa `approved_qty` bazında mı tetikleneceği implementasyonda netleşecek. Öneri: `received_qty ≥ approved_qty` → closed.
   ...
   **Kapanış (2026-07-04):** `received` geçişi miktar-bazlı yapıldı — `ShipmentService.Receive`, bir shipment'ın kendi teslim alma olayında BTO'yu koşulsuz `received`'e taşımak yerine, önce BTO'nun **tüm** kalemlerinde `received_qty ≥ approved_qty` (yoksa `requested_qty`) olup olmadığını kontrol eder (`TransferOrderItemRepo.AllReceived`, tüm shipment'lar üzerinden kümülatif). Koşul sağlanmadan BTO mevcut durumunda (`shipped`) kalır — yeni bir ara durum eklenmedi. Bu, iç içe geçmiş (interleaved) kısmi teslimat sırasında (sevk1→teslim1→sevk2→teslim2) ikinci shipment'ın `Advance`'inin BTO'yu artık `shipped` olmayan bir durumda bulup 409 ile reddedilmesi hatasını kapatır (`allowedBTOTransitions[received] = {closed}` idi, `shipped` geçişine izin vermiyordu). Bkz. `service/shipment_service.go` `Receive`, regresyon testi `service/integration_test.go` `TestTransferOrder_InterleavedShipments_SecondAdvanceNotBlockedByFirstReceive`.
