import { useTenantModules } from "@/hooks/use-tenant"
...
export function resolveEnabledModules(tenantModules: string[] | undefined): ModuleKey[] {
  // Before the tenant's modules have loaded — or on any fetch error — we show
  // every mounted module rather than an empty sidebar: flicker (or a
  // permanently missing section on a 403) on first paint would be worse than
  // a brief over-inclusion, and nothing here is an authorization boundary.
...
export function useEnabledModules(tenantId: string): ModuleKey[] {
  const { data } = useTenantModules(tenantId)
